...
class UsageCSVLogger(pl.Callback):
    def __init__(self, output_dir: str):
        super().__init__()
        self.csv_path = Path(output_dir) / "usage_log.csv"
        # write header once
        with open(self.csv_path, "w", newline="") as f:
            ...
            writer.writerow([
                ...
                "gpu_alloc_MB", "gpu_resr_MB",
                "cpu_pct"
            ])

    def _gather(self, trainer):
        total, used, free = shutil.disk_usage(trainer.default_root_dir)
    ...
    def on_train_batch_end(
        ...
        entry = self._gather(trainer)
        # now use the correct key "global_step"
...
def main(args=None, model=None) -> GenerativeQAModule:
    ...
    usage_csv_logger = UsageCSVLogger(output_dir=args.output_dir)
    args.callbacks = [usage_csv_logger]
    ...
    trainer: pl.Trainer = generic_train(
        ...
        callbacks=[UsageCSVLogger(output_dir=args.output_dir)],
        profiler=AdvancedProfiler() if args.profile else None,
        )

    print("num_sanity_val_steps:", trainer.num_sanity_val_steps, flush=True)
    print("val_check_interval:", trainer.val_check_interval, type(trainer.val_check_interval), flush=True)
    print("check_val_every_n_epoch:", trainer.check_val_every_n_epoch, flush=True)
    ...
    print(f"[USAGE] step={trainer.global_step} peak_gpu_alloc={peak:.1f}MB")
